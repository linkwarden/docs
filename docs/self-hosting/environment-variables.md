...
| Apple                  | `NEXT_PUBLIC_APPLE_ENABLED`<br />`APPLE_CUSTOM_NAME`<br />`APPLE_CLIENT_ID`<br />`APPLE_TEAM_ID`<br />`APPLE_KEY_ID`<br />`APPLE_PRIVATE_KEY`                                   | Enables Apple login, optionally renames the button, and sets the Services ID plus the Sign in with Apple key (team ID, key ID, and .p8 key) used to generate the client secret automatically. |
