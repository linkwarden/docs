...
After changing your `.env` file, run `docker compose up -d` to apply the changes.
...
| `BROWSER_LIFECYCLE`                         | `on-demand`        | How the archiving browser is managed. `on-demand` launches it when work arrives and closes it after 60 seconds of an empty queue; The other option is `persistent`, which keeps a browser running for the life of the worker. |
