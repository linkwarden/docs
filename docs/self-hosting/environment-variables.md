...
| `BROWSER_LIFECYCLE`                         | `on-demand`        | How the archiving browser is managed. `on-demand` launches it when work arrives and closes it after 60 seconds of an empty queue; any other value keeps one browser running for the life of the worker. |
